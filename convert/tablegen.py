...
sys.stdout.write('const uint8_t sclera[SCLERA_HEIGHT][SCLERA_WIDTH] = {')
...
for y in range(IMAGE.size[1]):
    for x in range(IMAGE.size[0]):
        ...
        HEX.write(
            ((p[0] & 0b11100000) << 0) | # Convert 24-bit RGB
            ((p[1] & 0b11100000) >> 3) | # to 8-bit value w/
            ((p[2] & 0b11000000) >> 6))  # 3/3/2-bit packing
...
sys.stdout.write('const uint8_t iris[IRIS_MAP_HEIGHT][IRIS_MAP_WIDTH] = {')
